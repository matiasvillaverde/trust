...
SELECT
    id, broker_order_id, created_at, updated_at, deleted_at,
    unit_price, currency, CAST(quantity AS TEXT), category, trading_vehicle_id,
    action, status, time_in_force, trailing_percentage, trailing_price,
    CASE WHEN filled_quantity IS NULL THEN NULL ELSE CAST(filled_quantity AS TEXT) END,
