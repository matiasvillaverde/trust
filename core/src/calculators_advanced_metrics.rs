...
/// Pareto-style concentration metrics for realized profits.
#[derive(Debug, Clone, PartialEq)]
pub struct ProfitConcentrationMetrics {
    /// Share of gross profit generated by the top 20% of winning trades.
    pub top_20pct_profit_share_percentage: Decimal,
    /// Share of closed trades needed to explain 80% of gross profit.
    pub trade_share_to_reach_80pct_profit_percentage: Decimal,
}
...
impl AdvancedMetricsCalculator {
    ...
    /// Calculate profit factor as `gross_profit / gross_loss_abs`.
    ///
    ...
    /// Calculate expectancy (EV) as
    /// `(win_rate * average_win) - (loss_rate * average_loss)`.
    ///
    /// This is equivalent to mean realized PnL per closed trade.
    ///
    /// # Arguments
    /// * `closed_trades` - Vector of closed trades to analyze
    ///
    ...
    /// Calculate Pareto-style profit concentration for closed trades.
    ///
    /// This is not a standardized industry ratio. It intentionally exposes
    /// explicit concentration fields instead of a single ambiguous "Pareto Index".
    pub fn calculate_profit_concentration_metrics(
        closed_trades: &[Trade],
    ) -> ProfitConcentrationMetrics {
        let mut winning_pnls: Vec<Decimal> = closed_trades
            .iter()
            .filter(|trade| trade.balance.total_performance > dec!(0))
            .map(|trade| trade.balance.total_performance)
            .collect();

        if winning_pnls.is_empty() || closed_trades.is_empty() {
            return ProfitConcentrationMetrics {
                top_20pct_profit_share_percentage: dec!(0),
                trade_share_to_reach_80pct_profit_percentage: dec!(0),
            };
        }

        winning_pnls.sort_by(|a, b| b.cmp(a));

        let gross_profit = winning_pnls.iter().copied().sum::<Decimal>();
        ...
        let top_bucket_len = (winning_pnls.len().saturating_add(4)) / 5;
        let top_bucket_profit = winning_pnls
            .iter()
            .take(top_bucket_len)
            .copied()
            .sum::<Decimal>();
        let top_20pct_profit_share_percentage = top_bucket_profit
            .checked_mul(dec!(100))
            .and_then(|value| value.checked_div(gross_profit))
            .unwrap_or(dec!(0));

        let target_profit = gross_profit.checked_mul(dec!(0.8)).unwrap_or(dec!(0));
        let mut cumulative_profit = dec!(0);
        let mut trades_needed = 0usize;

        for pnl in winning_pnls {
            cumulative_profit = cumulative_profit
                .checked_add(pnl)
                .unwrap_or(cumulative_profit);
            trades_needed = trades_needed.saturating_add(1);
            if cumulative_profit >= target_profit {
                break;
            }
        }

        let trade_share_to_reach_80pct_profit_percentage = Decimal::from(trades_needed)
            .checked_mul(dec!(100))
            .and_then(|value| value.checked_div(Decimal::from(closed_trades.len())))
            .unwrap_or(dec!(0));
    ...
    /// `annualized_return_percentage / max_drawdown_percentage`.
    ///
    /// `starting_equity` is the baseline capital for the observed window and
    /// `period_days` is the length of that window in days.
    pub fn calculate_calmar_ratio_from_equity(
        net_profit: Decimal,
        starting_equity: Decimal,
        period_days: i64,
        max_drawdown_percentage: Decimal,
    ) -> Option<Decimal> {
        if starting_equity <= dec!(0) || period_days <= 0 || max_drawdown_percentage <= dec!(0) {
            return None;
        }

        let total_return_percentage = net_profit
            .checked_mul(dec!(100))
            .and_then(|value| value.checked_div(starting_equity))?;
        let annualization_factor = Decimal::from(365).checked_div(Decimal::from(period_days))?;
        let annualized_return_percentage =
            total_return_percentage.checked_mul(annualization_factor)?;

        annualized_return_percentage.checked_div(max_drawdown_percentage)
    }

    fn calculate_trade_span_days(closed_trades: &[Trade]) -> Option<i64> {
        let start = closed_trades.iter().map(|trade| trade.created_at).min()?;
        let end = closed_trades.iter().map(|trade| trade.updated_at).max()?;
        ...
        Some((seconds.saturating_add(86_399)) / 86_400)
    }

    /// Calculate a Calmar ratio approximation from trades plus realized
    /// drawdown inputs supplied by the caller.
    pub fn calculate_calmar_ratio_for_report(
        closed_trades: &[Trade],
        starting_equity: Decimal,
        max_drawdown_percentage: Decimal,
    ) -> Option<Decimal> {
        let period_days = Self::calculate_trade_span_days(closed_trades)?;
        let net_profit = Self::calculate_net_profit(closed_trades);
        Self::calculate_calmar_ratio_from_equity(
            net_profit,
            starting_equity,
            period_days,
    ...
    ///
    /// This fallback uses cumulative closed-trade PnL drawdown amount and is
    /// retained for surfaces that do not have realized equity-curve inputs.
...
mod tests {
    ...
    fn create_dated_test_trade(performance: Decimal, day_offset: i64) -> Trade {
        let mut trade = create_test_trade(performance);
        let base = Utc::now().naive_utc();
        trade.created_at = base + Duration::days(day_offset);
        trade.updated_at = trade.created_at + Duration::hours(6);
        trade
    }
    ...
    #[test]
    fn test_calculate_profit_concentration_metrics_empty_trades() {
        let metrics = AdvancedMetricsCalculator::calculate_profit_concentration_metrics(&[]);
        assert_eq!(metrics.top_20pct_profit_share_percentage, dec!(0));
        assert_eq!(
            metrics.trade_share_to_reach_80pct_profit_percentage,
            dec!(0)
        );
    }

    #[test]
    fn test_calculate_profit_concentration_metrics_no_winners() {
        let trades = vec![create_test_trade(dec!(-100)), create_test_trade(dec!(-50))];
        let metrics = AdvancedMetricsCalculator::calculate_profit_concentration_metrics(&trades);
        assert_eq!(metrics.top_20pct_profit_share_percentage, dec!(0));
        assert_eq!(
            metrics.trade_share_to_reach_80pct_profit_percentage,
            dec!(0)
    ...
    #[test]
    fn test_calculate_profit_concentration_metrics_concentrated_profits() {
        let trades = vec![
            create_test_trade(dec!(400)),
            create_test_trade(dec!(100)),
            create_test_trade(dec!(50)),
        ...
        // Top 20% winners = top 1 winner = 400 / 575 = 69.57%
        assert!(
            (metrics.top_20pct_profit_share_percentage - dec!(69.56521739)).abs() < dec!(0.0001)
        );
        // Need 2 of 5 closed trades to exceed 80% of gross profit.
        assert_eq!(
            metrics.trade_share_to_reach_80pct_profit_percentage,
            dec!(40)
        );
    ...
    #[test]
    fn test_calculate_profit_concentration_metrics_uniform_profits() {
        let trades = vec![
            create_test_trade(dec!(100)),
            create_test_trade(dec!(100)),
            create_test_trade(dec!(100)),
        ...
        assert_eq!(metrics.top_20pct_profit_share_percentage, dec!(20));
        assert_eq!(
            metrics.trade_share_to_reach_80pct_profit_percentage,
            dec!(80)
        );
    ...
    #[test]
    fn test_calculate_calmar_ratio_from_equity_rejects_invalid_inputs() {
        assert_eq!(
            AdvancedMetricsCalculator::calculate_calmar_ratio_from_equity(
                dec!(100),
                dec!(0),
                30,
                dec!(10),
    ...
    #[test]
    fn test_calculate_calmar_ratio_from_equity_annualizes_return() {
        let calmar = AdvancedMetricsCalculator::calculate_calmar_ratio_from_equity(
            dec!(120),
            dec!(1000),
            30,
            dec!(10),
        ...
        // 12% over 30 days annualized linearly => 146%, divided by 10% drawdown = 14.6
        assert!((calmar - dec!(14.6)).abs() < dec!(0.0001));
    }

    #[test]
    fn test_calculate_calmar_ratio_for_report_uses_trade_window() {
        let trades = vec![
            create_dated_test_trade(dec!(100), 0),
            create_dated_test_trade(dec!(-50), 10),
            create_dated_test_trade(dec!(150), 20),
        ];

        let calmar = AdvancedMetricsCalculator::calculate_calmar_ratio_for_report(
            &trades,
            dec!(1000),
            dec!(8),
        )
        .expect("report calmar ratio");

        // Net profit = 200 => 20% over 21 observed days => 347.619...% annualized, / 8 = 43.45...
        assert!((calmar - dec!(43.45238095)).abs() < dec!(0.0001));
    }

    #[test]
    fn test_calculate_calmar_ratio_for_report_requires_positive_time_span() {
        let base = Utc::now().naive_utc();
        let mut trade = create_test_trade(dec!(100));
        trade.created_at = base;
        trade.updated_at = base;

        assert_eq!(
            AdvancedMetricsCalculator::calculate_calmar_ratio_for_report(
                &[trade],
                dec!(1000),
                dec!(10),
            ),
